...
export async function incrementViewCount(slug: string) {
  ...
  try {
    ...
    const msg = error instanceof Error ? error.message : (error as any)?.message ?? (error as any)?.code ?? "unknown";
    console.warn("View count unavailable (table may not exist yet):", msg);
    return 0;
  }
}
...
export async function getArticleReactions(slug: string) {
  ...
  try {
    ...
    const msg = error instanceof Error ? error.message : (error as any)?.message ?? (error as any)?.code ?? "unknown";
    console.warn("Reactions unavailable (table may not exist yet):", msg);
    return {};
  }
}
...
export async function toggleReaction(slug: string, reactionType: ReactionType) {
  ...
  try {
    ...
    const msg = error instanceof Error ? error.message : (error as any)?.message ?? (error as any)?.code ?? "unknown";
    console.warn("Toggle reaction failed:", msg);
    return {
      success: false,
